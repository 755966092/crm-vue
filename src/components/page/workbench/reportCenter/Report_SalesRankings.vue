  ...
  data() {
    return {
     props: {
      ...
      histogramDate: ["杨慧", "何西楠", "王娜", "周柏丞", "王佳伟", "王松柏"],
      // 柱状图数据
      histogramData: {
  ...
  methods: {
   // 获取公司部门
    ...
    filterClue() {
      ...
      self
        .$axios({
          method: "POST",
          withCredentials: false,
          url: "/api/reportCenter/RtfJournaling",
          data: obj
        })
        .then(function(res) {
          if (res.data.code === 200) {
    ...
    drawLine() {
      ...
      console.log(self.histogramData.clientFrequency.sort((a, b) => a > b));
      // 绘制图表
      myChart.setOption({
        ...
        series: [
          {
            ...
            barWidth: 15,
            type: "bar",
            data: self.histogramData.clientFrequency.sort((a, b) => b > a)
          }
        ]
      });
    },// 请求当前用户所属公司所有部门
